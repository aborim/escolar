...
<div class="content_aluno">
    <div class="titulo">Exibindo Informa&ccedil; &otilde;es do Coordenador: <?= $nomeCoordenador;?></div>
    <div id="DadosCoordenador">
    <?php
    ...
    $buscar=$con->prepare("SELECT endereco.endereco,
        ...
        coordenador.cpf,coordenador.rg,
        coordenador.ctps,coordenador.pis,
        coordenador.ctpsDtExpedicao,
        coordenador.dtAdmissao,
        coordenador.telefone,coordenador.celular,coordenador.dtNascimento,
        coordenador.observacao, coordenador.dtCriacao,usuarios.usuario
FROM 
		coordenador,endereco,usuarios 
WHERE 	
		coordenador.idUsuario=usuarios.id and 
        coordenador.idEndereco=endereco.id AND
        coordenador.id=:idCoordenador");
    $buscar->bindValue(":idCoordenador",$idCoordenador);
    $buscar->execute();
    
    if ($buscar->rowCount()!=1) {
        echo "Coordenador n &atilde;o encontrado!";
    } else {
